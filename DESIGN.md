...
한 계층씩 올라간다. **버튼은 안 올린다** — `ActionButton` 은 15 medium 그대로다.
버튼은 읽는 글이 아니라 누르는 면이고, 높이(50)가 이미 크기를 말한다.

28 은 **상세 시트 머리의 금액 하나만** 쓴다. 한 화면에 하나뿐이라 크기를 독점시킬
...
위로도 마찬가지다. `.largeTitle`(34) · `.body`(17) · `.callout`(16) ·
`.footnote`(13)는 안 쓴다. `.title`(28)은 `heroAmount()` 한 곳만 쓴다.
...
- **시트 바닥의 큰 버튼**은 `ActionButton`. 가로를 채우고 높이는
  `DS.Size.actionButton`(50), 글자는 15 medium 이다. 종류는 넷뿐이다 —
