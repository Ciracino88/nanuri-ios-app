...
화면을 만들거나 고치기 전에는 **`DESIGN.md`** 를 본다. 값은
`Components/DesignSystem.swift` 의 `DS`(뜻이 붙은 시맨틱 층)와
`Components/DesignTokens.swift` 의 `Ramp`(원시 팔레트) 두 겹으로 나뉘어 있고,
**화면은 시맨틱 층만 쓴다. 화면 코드에 숫자를 직접 적지 않는다.**
시각 언어의 출처는 `TOSS.md` 다 — 값의 근거가 필요하면 거기를 보고,
이 앱에서 그걸 어떻게 쓰기로 했는지는 `DESIGN.md` 에 있다.
...
- 헤더 통일 · 프로필 탭 · 알림함(2026-08-16), 그리고 묶어 보내기 선택 모드 ·
  당겨서 새로고침 · 이미지 캐시(2026-08-19)까지 **실기기에서 동작을 확인했다.**
- 안 본 것은 둘 남았다 — **다크 모드**와 **큰 글씨(Dynamic Type)**. 평소 설정과 다른
  조건이라 따로 켜 봐야 한다 (`DESIGN.md` 맨 아래 1·2번).
