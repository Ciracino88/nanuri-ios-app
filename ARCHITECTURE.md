...
flowchart TD
    ...
    CHECK --> SP["항목 3개<br/>수영장 260,000<br/><b>카페모임 138,000</b><br/>파라솔 60,000"]

    TX --- SP
```

**완전한 역연산이 아니다.** 4건이 3항목으로 돌아온다 — `ledgerLines` 가 같은 제목을
...
거래는 **통장에 찍힌 그대로** 넣고(적요도 은행 값), 장부에 적힐 항목은 **분할**로
만든다. 청구가 하나뿐일 때도 분할을 만든다 — 그래야 은행 적요를 덮어쓰지 않고
항목에 제 이름을 줄 수 있다. **같은 제목의 청구는 한 항목으로 합친다**
(이승호 청구 4건 → 항목 3개, 카페모임 110,500 + 27,500 = 138,000).
...
같을 수 있어서다. 그리고 **항목(분할)도 만들지 않는다** — 사람 장부에 없는 줄이다.

**거래내역서 거래 편집에선 사람이 끌 수 있다.** 이름이 우연히 같아 오탐이 날 수 있고,
잠가 두면 빠져나갈 길이 없다. (수기 거래엔 토글이 없다 — 이체는 내역서로만 들어오니까.)
...
| | 적요·카테고리·분할·영수증 | 금액·일시·통장 |
| --- | --- | --- |
| `statement` (거래내역서에서 불러옴) | 고칠 수 있다 | **잠근다** |
| `manual` (사람이 앱에서 넣음) | 고칠 수 있다 | 고칠 수 있다 |

**`counter_account_id`(이체 표시)는 이 표 밖이다 — 매처가 세우되, 거래내역서 거래
편집에선 사람이 고칠 수 있다.** "이게 통장 사이 이체인가" 는 거래내역서에 없는
정보라, 불러올 때 **적요가 다른 통장의 `statement_alias` 와 같은가** 로 매처가
판정한다(위 "통장 사이 이체는 거래내역서로 들어온다"). 그 적요는 곧 상대 예금주라
매칭은 대개 정확하지만, **은행이 말해 줄 수 없는 회계 판단**이라 못 잡거나 오탐일 때
사람이 마지막에 고쳐야 한다 — 그래서 **거래내역서 거래 편집에 통장 사이 이체 토글을
남긴다.**

2026-09-03 에 실제로 필요했다. 8월 모임 22건이 `statement_alias` 판정보다 먼저 들어가
있어서 400만원 예산 수령 줄에 `counter_account_id` 가 없었고(다시 불러와도 "이미 장부에
있어요" 로 걸러짐), 편집에서 사람이 켜서 바로잡았다. 안 그랬으면 농협 잔액이 400만원
많고 총 재정도 그만큼 부풀어 있었다.

**단, 수기 거래(추가·편집)에는 토글이 없다.** 내부 이체는 반드시 모임통장을 지나
거래내역서로 들어오므로 손입력이 이체일 수 없고, 수기로 적으면 오히려 같은 사건이 두
줄이 된다. 그래서 토글이 뜨는 자리는 **거래내역서 거래 편집** 하나다.
...
한 항목, 없으면 거래 자체다. 묶어 보낸 출금 하나가 조각 아홉이면 아홉 항목이다.

예전에는 거래를 그렸고, 그래서 **목록만 통장 시점이고 보고서·분석은 장부 시점**
이라 한 앱 안에 단위가 둘이었다. 사람이 쓰던 엑셀도 항목 단위다.

**통장 대조를 잃지 않는다.** 중복 방지(`StatementMatcher`)도 잔액 검산도
**데이터가 하는 일**이라 화면이 어떻게 그려지든 그대로고, 통장을 나란히 놓고 보는
자리는 거래내역서 확인 화면이 맡는다.

**내부 이체는 안 쪼갠다.** 사람 장부에 없는 줄이라 조각이 애초에 없다 — 목록엔 한 항목으로 남는다.

조각인 항목을 눌러도 **부모 거래**의 편집이 열린다. 고칠 것이 항목 하나여도 금액·통장·
영수증은 거래가 갖고 있다.
