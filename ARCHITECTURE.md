...
농협은 인터넷뱅킹이 없어 송금 한 번에 ATM 을 가야 한다. 그래서 청구를 **실시간으로
처리할 통장**을 따로 둬야 했다. 그 자리에 토스뱅크 모임통장을 앉힌 이유가 셋이다.

- **은행 무관·무수수료 입출금.** 청구자 계좌가 어느 은행이든 수수료 없이 보낸다.
- **딥링크 송금이 빠르고 간편하다.** 앱이 금액·계좌를 채운 채 토스를 열어, 사람은
  확인만 한다 (`supertoss://send`).
- **거래내역서를 내보낼 수 있다.** 결산 때 통장 기록을 앱으로 가져와 청구와
  대조·전산화할 수 있다 — 이 한 가지가 장부 반자동화의 전제다.

매달 농협에서 예산을 넘겨 두고 지출은 모임통장에서 낸다. 헌금은 모임통장이 수령처로
승인나지 않아 **농협으로만** 들어온다. 그래서 통장이 둘이고 장부가 하나인 구조가 된다
(아래 "통장은 둘, 장부는 하나").
...
단계마다 데이터 단위가 다르다. 이걸 뭉뚱그리면 매칭도 잔액도 어긋난다.

| 단위 | 뜻 | 사는 곳 |
| --- | --- | --- |
| **청구 (`Bill`)** | 청구자가 제출한 청구 1건 | DB `bills` |
| **송금 묶음 (`BillGroup`)** | 같은 사람의 여러 청구를 합친 송금 1건 | **저장 안 함.** `processed_at` + 이름으로 재구성 (`StatementMatcher.groups(from:)`) |
| **거래 (`finance_transactions`)** | 은행이 증명한 사실. 토스 내역서 한 줄 | DB `finance_transactions` |
| **항목 (`finance_items`)** | 사람이 관리하는 장부 정본 | DB `finance_items` |

- **`BillGroup` 은 표가 아니다.** 같은 승인 요청으로 처리된 청구들이 같은
  `processed_at`(트리거가 `now()` 로 찍음)을 갖는 걸 이용해 그때그때 묶어 세운다.
  묶음의 합계가 곧 토스에서 빠진 한 줄이다.
- **거래와 항목은 다른 개념이다.** 거래는 파싱으로만 생기고 편집하지 않는(은행 증명),
  항목은 사람의 판단(카테고리·적요·영수증·분할)이다. 한 거래가 여러 항목으로 풀린다.

넷이 어긋나는 대표 사례가 **청구 4 → 송금 1 → 거래 1 → 항목 3** 이다. 어떻게 묶고
되푸는지는 아래 "거래내역서는 청구서와 맞춰서 들어온다" 에 있다.
...
flowchart TD
    ...
    CHECK --> TX["거래 1건 (은행 증명)<br/>−458,000 · 은행 적요 '이승호'"]
    CHECK --> SP["항목 3개 (장부)<br/>수영장 260,000 · 영수증 1장<br/><b>카페모임 138,000 · 영수증 2장</b><br/>파라솔 60,000 · 영수증 1장"]
