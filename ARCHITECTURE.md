...
통장이 **토스 모임통장 하나**다. 그래서 장부도 하나고, 재정 탭은 **열자마자 그 장부를
연다** — `FinanceViewModel.start()` 가 목록을 받아 바로 선택한다.
...
**스키마는 안 건드렸다.** `finance_transactions.ledger_id` 는 `unique (ledger_id,
datetime, amount)` 의 일부라 빼면 중복 방지가 무너지고, 이미 데이터가 들어 있다.
장부라는 **개념**은 DB 에 남고 **고르는 화면**만 없앤 것이다.
