...
struct AdminHeaderView<Leading: View, Trailing: View>: View {
    ...
    /// 가운데를 이름이 아닌 것으로 바꾼다 (재정 탭의 달 넘김).
    ///
    /// 탭 이름은 이미 탭바가 말하고 있어서, 그 자리가 더 쓸모 있는 화면이면
    ...
    /// 안 읽은 알림은 점 하나로만 알린다. 색은 주황(주의)이다.
    /// **파랑을 쓰지 않는다** — 파랑은 화면당 하나뿐인 주요 동작에 예약돼 있다.
